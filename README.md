...
## Две секции в одном репозитории

Это конвейер коротких видео, разбитый на независимые части. Каждую можно
поставить отдельно.

| Секция | Что делает | Где |
|---|---|---|
| **Монтаж** | сырое видео → готовый к публикации ролик: речь, склейки, субтитры, графика, звук, рендер | корень репозитория, ниже по этому файлу |
| **Тренды** | что работает в нише и что болит у аудитории → пакет сценаристу и банки деталей | [trends/](trends/README.md) |

Порядок в производстве такой: тренды дают идею — что показать, сценарист
превращает её в задание, монтаж собирает готовый ролик.

Секции не связаны кодом и ставятся по отдельности: анализ работает без
монтажа, монтаж — без анализа.
